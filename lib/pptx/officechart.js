...
function OfficeChart(chartInfo) {
  ...
  return {
    ...
    setData: function (data) {
      ...
      if (data) {
        ...
        if (chartInfo.data) {
          ...
          // OOXML requires c:ser elements before c:axId — extract axIds, push series, re-append axIds
          var axIdElements = []
          for (var j = seriesDataRef.length - 1; j >= 0; j--) {
            if (seriesDataRef[j]['c:axId']) {
              axIdElements.unshift(seriesDataRef.splice(j, 1)[0])
            }
          }
          for (var i = 0; i < seriesData.length; i++) {
            seriesDataRef.push(seriesData[i])
          }
          for (var j = 0; j < axIdElements.length; j++) {
            seriesDataRef.push(axIdElements[j])
          }
        }
      }
      return this
    ...
    // Element ordering follows OOXML spec (ISO/IEC 29500):
    //   CT_LineSer:  idx, order, tx, spPr, marker, dPt, dLbls, trendline, errBars, cat, val, smooth
    //   CT_BarSer:   idx, order, tx, spPr, invertIfNegative, dPt, dLbls, trendline, errBars, cat, val
    //   CT_PieSer:   idx, order, tx, spPr, dPt, dLbls, cat, val
    _ser: function (serie, i) {
      var rc2a = this._rowColToSheetAddress
      var isBarType = ['bar', 'column', 'stacked-column', 'group-bar'].indexOf(chartInfo.renderType) !== -1

      var ser = {}
      ser['c:idx'] = { '@val': i }
      ser['c:order'] = { '@val': i }
      ser['c:tx'] = this._strRef('Sheet1!' + rc2a(1, 2 + i, true, true), [
        serie.name
      ])
      ...
      if (isBarType) {
        ser['c:invertIfNegative'] = { '@val': '0' }
      }

      if (serie.xml && serie.xml['c:marker']) {
        ser['c:marker'] = serie.xml['c:marker']
      }

      ser['c:cat'] = this._strRef(
        'Sheet1!' +
          rc2a(2, 1, true, true) +
          ':' +
      ...
      if (serie.xml && serie.xml['c:smooth']) {
        ser['c:smooth'] = serie.xml['c:smooth']
      }

      if (serie.xml) {
        var xmlCopy = Object.assign({}, serie.xml)
        delete xmlCopy['c:marker']
        delete xmlCopy['c:smooth']
        if (Object.keys(xmlCopy).length > 0) {
          ser = _.merge(ser, xmlCopy)
        }
      }

      if (serie.colors) {
        ser['#text'] = this._colorRef(serie.colors)
      }
